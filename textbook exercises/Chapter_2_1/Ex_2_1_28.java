...
public class Ex_2_1_28 {
    public static double time(String algo, int size, int N) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            Integer[] a = getTwoKeyArray(size); // size: size of array
    ...
    private static Integer[] getTwoKeyArray(int size) {
        Integer[] a = new Integer[size];
        int[] values = { 1, 2 };
        Random rand = new Random();
        for (int i = 0; i < size; i++) {
            a[i] = values[rand.nextInt(values.length)];
        }
        return a;
    }
    ...
    public static void main(String[] args) {
        ...
        StdOut.printf("Selection time: %.6fs\n", selectionTime);
        StdOut.printf("Insertion time: %.6fs\n", insertionTime);
        StdOut.printf(
                "The array with two distinct elements: array size is %2d, run %2d times, selection sort use %.3f times than insertion sort\n",
